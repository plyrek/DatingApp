...
export class MemberMessagesComponent implements OnInit {
      ...
      tap(messages => {
        for (let i = 0; i < messages.length; i++) {
          if (messages[i].isRead === false && messages[i].recipientId === currentUserId) {
            this.userServices.markAsRead(currentUserId, messages[i].id);
          }
        }
      })
    )
  ...
  sendMessage() {
    ...
    this.userServices.sendMessage(this.authService.decodedToken.nameid, this.newMessage).subscribe( (message: Message) => {
